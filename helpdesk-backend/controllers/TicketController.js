...
const createTicket = async (req, res) => {
    try {
        const { Issue, description, category, priority } = req.body;
        const userId = req.user.userId; // Extracted from JWT by authenticate middleware

        if (!Issue || !description || !category || !priority) {
            return res.status(400).json({ message: "All fields (Issue, description, category, priority) are required." });
        }

        const newTicket = new Ticket({
            // ticketId is generated by default by the model
            userId,
            Issue,
            description,
            category,
        ...
        // Optional: Add a system comment that the ticket was created
        await Comment.create({
            ticketId: newTicket.ticketId,
            userId: 'system', // Or req.user.userId if user creates the first "comment"
